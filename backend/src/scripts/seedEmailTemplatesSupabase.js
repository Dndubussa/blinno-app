...
// Try to import Supabase, but handle case where environment variables are not set
let supabase;
try {
  const supabaseModule = await import('../config/supabase.js');
  supabase = supabaseModule.supabase;
} catch (error) {
  console.warn('Supabase not configured. Running in preview mode only.');
  supabase = null;
}
...
async function seedEmailTemplates() {
  ...
  // If Supabase is not available, just show what would be seeded
  if (!supabase) {
    console.log('Preview mode - showing templates that would be seeded:');
    console.log('');
    ...
    for (const template of templates) {
      try {
        ...
        console.error(`Error reading template ${template.name}:`, error.message);
      }
    }
    
    console.log('Email template seeding preview completed!');
    console.log('To actually seed templates, configure Supabase environment variables.');
    return;
  }
